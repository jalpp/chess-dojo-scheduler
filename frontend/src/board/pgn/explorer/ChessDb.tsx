...
export function ChessDBTab({ moves, loading, error, requestAnalysis }: ChessDBTabProps) {
    ...
    return (
        <Stack mt={2} spacing={1}>
            {moves.map((move, i) => {
                const widthPct = totalGames > 0
                    ? Math.max(10, (totalGames - i) / totalGames * 100)
                    : 0;
                ...
                return (
                    ...
                    >
                        ...
                        >
                            <Typography sx={{ minWidth: 50, fontWeight: move === bestMove ? 700 : 400 }}>
                                {move.san}
                            </Typography>
                            <Box sx={{ flex: 1, bgcolor: 'divider', borderRadius: 1, height: 8 }}>
                                <Box sx={{ width: `${widthPct}%`, bgcolor: 'primary.main', height: '100%', borderRadius: 1 }} />
                            </Box>
                            <Typography sx={{ minWidth: 45, textAlign: 'right', fontSize: '0.8rem' }}>
                                {move.score}
                            </Typography>
                            <Typography sx={{ minWidth: 40, textAlign: 'right', fontSize: '0.8rem', color: 'text.secondary' }}>
                                {move.winrate}%
                            </Typography>
                        </Box>
                    </Tooltip>
